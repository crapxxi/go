...
func putProduct(w http.ResponseWriter, r *http.Request) {
	...
	if role == "admin" {
		...
		_, err := db.Exec("UPDATE products SET name = CASE WHEN $1 = '' THEN name ELSE $1 END, description = CASE WHEN $2 = '' THEN description ELSE $2 END, price = CASE WHEN $3 = 0 THEN price ELSE $3 END, stock = CASE WHEN $4 = 0 THEN stock ELSE $4 END, image = CASE WHEN $5 = '' THEN image ELSE $5 END WHERE id = $6", p.Name, p.Description, p.Price, p.Stock, p.Image, id)
		if err != nil {
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			log.Print(err)
			return
